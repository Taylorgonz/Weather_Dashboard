...
let id = 0;
const fiveDayArr = [3, 11, 19, 27, 35];
...
$('#input-form').append('<button id="search" type="button" class="search button is-primary"> Search </button> <div class="column"> <input id="city-value" class="city-value input is-primary" type="text" placeholder="City"/> </div>')
...
function weatherCall() {
  ...
  $.get("https://api.openweathermap.org/data/2.5/weather?q=" + cityInput + "&units=imperial&appid=" + myKey + "" ).then(function(response) {
    ...
    let weatherDisplay = $(
      '<div class=" current-weather card-content" style="background-color: hsl(171, 100%, 41%); border-radius: 10px;">'
    );
  ...
  $.get(
    "https://api.openweathermap.org/data/2.5/forecast?q=" + cityInput + "&" + stateInput + "&units=imperial&appid=" + myKey +"").then(function (response) {
    // console.log(response);
    
    for (let i = fiveDayArr[0]; i < fiveDayArr.length ; i ++) {
      // console.log(response.list[i]);

      // fiveDayArr.forEach((response) => {

      // if (response.list === fiveDayArr ) {
      ...
      const temp = $("<p>").text(
        "Temperature: " + Math.floor(response.list[i].main.temp)
      );
      const humidity = $("<p>").text(
        "Humdity: " + response.list[i].main.humidity
      );
      // console.log(date)
      // console.log(icon)
...
function renderButtons() {
  ...
  for (let i = 0; i < buttonInput.length; i++) {
    ...
    button
      ...
      .text("" + buttonInput[i].city + ", " + buttonInput[i].state + "");
    // console.log(button);
